...
The current image is based on the [openjdk:8-jdk-alpine image](https://hub.docker.com/_/openjdk/), and adds [Terracotta 4.3.5 OSS on top of it](http://terracotta.org/downloads/open-source/catalog)

#### How to start your Terracotta Server(s) in Docker containers

##### Quick start : one active node

    docker run --name tc-server -p 9510:9510 -d terracotta/terracotta-server-oss:4.3.5
...
To build this [Dockerfile](https://github.com/Terracotta-OSS/docker/blob/master/4.3.5/server/Dockerfile), clone this [git repository](https://github.com/Terracotta-OSS/docker) and run :
    ...
    $ cd 4.3.5/server
    $ docker build -t terracotta-server-oss:4.3.5 .
...
##### Starting the containers !
    ...
    $ docker run --hostname tsa --name tsa -d -e TC_SERVER1=tsa -e TC_SERVER2=tsa2 --net=my-net --env="constraint:node==mhs-demo1" terracotta/terracotta-server-oss:4.3.5
    $ docker run --hostname tsa2 --name tsa2 -d -e TC_SERVER2=tsa2 -e TC_SERVER1=tsa --net=my-net --env="constraint:node==mhs-demo2" terracotta/terracotta-server-oss:4.3.5
    $ docker run --name petclinic -d --net=my-net --env="constraint:node==mhs-demo0" terracotta/sample-ehcache-client:4.3.5

You should end up with something similar to :
````
$ docker ps
CONTAINER ID        IMAGE                                             COMMAND                  CREATED             STATUS              PORTS                           NAMES
8dbf24cfaae3        terracotta/sample-ehcache-client:4.3.5   "mvn tomcat7:run -Dts"   56 seconds ago      Up 55 seconds                                            mhs-demo0/petclinic
b7e10058a909        terracotta/terracotta-server-oss:4.3.5        "/bin/sh -c 'sed -i -"   3 minutes ago       Up 3 minutes        9510/tcp, 9530/tcp, 9540/tcp    mhs-demo1/tsa2
8999a50ef131        terracotta/terracotta-server-oss:4.3.5        "/bin/sh -c 'sed -i -"   9 minutes ago       Up 9 minutes        9510/tcp, 9530/tcp, 9540/tcp    mhs-demo2/tsa
````
