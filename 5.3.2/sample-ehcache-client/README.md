...
You can start it up simply with :
    ...
    docker run --name client -d terracotta/sample-ehcache-client:5.3.2
...
and then re try running the client, with :
    ...
    docker run -d --link terracotta:terracotta --name client terracotta/sample-ehcache-client:5.3.2
...
To build this [Dockerfile](https://github.com/Terracotta-OSS/docker/blob/master/5.3.2/server/Dockerfile), clone this [git repository](https://github.com/Terracotta-OSS/docker) and run :
    ...
    $ cd 5.3.2/sample-ehcache-client
    $ docker build -t sample-ehcache-client:5.3.2 .